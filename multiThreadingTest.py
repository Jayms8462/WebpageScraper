...
url = [
    ...
    "https://www.tcgplayer.com/product/138959/metax-tcg-justice-league-1-strength-s65jl/shouldfail",
...
def main():
    with open('test.csv', 'a', newline = "", encoding="utf-8") as f:
        with concurrent.futures.ProcessPoolExecutor(max_workers = 12) as executor:
            for i in executor.map(scraping, url):
                write = csv.writer(f)
                write.writerows([i])
            f.close()
...
def scraping(url):
    ...
    try:
        ...
        data = [setName, card, image, description, dataAttributes]
        print('Completed:', url)
        return data
    except:
        print("There was an issue with:", url)
